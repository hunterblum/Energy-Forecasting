...
Create Results df
```{r}
Model <- c("Naive", "2-Hour MA", "Auto ARIMA")
Tr_RMSE <- c(Naive_rmse_tr, MA_rmse_tr, auto_arima_rmse_tr)
Val_RMSE <- c(Naive_rmse_val, MA_rmse_val, auto_arima_rmse_val)
...
ARIMA
```{r}
#take a look ACF plot with differenced
acf(diff(diff(X_tr)))
```
```{r}
...
```{r}
#Arima model
my_arima_model<-Arima(y_tr, order=c(3,2,2),xreg=X_tr)
summary(my_arima_model)
```

```{r}
#check the residuals
checkresiduals(my_arima_model)
```
Forcasting
```{r}
my_forcast<-forecast(my_arima_model, xreg = X_val)
my_forcast

#plot
autoplot(my_forcast, series = "Forecast") +
  coord_cartesian(xlim = c(38000, 39488))
```
Neural network
```{r}
#size= (p+p+1)/2 = (3+2+1)/2 =3
SD_NN <- nnetar(train_wine, p=3, q=2, size =3 )
summary(SD_NN)
```
```{r}
#prediction
SD_NN.pred<-forecast(SD_NN, h = 30)
```
```{r}
