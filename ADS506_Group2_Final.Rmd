...
```{r}
M = cor(num)
corrplot(M, addCoef.col = 'black', main = "Figure 1.2: Correlation Between Numeric Features", type = "lower")
```
...
M2 = cor(num2)
corrplot(M2, addCoef.col = 'black', type = "lower", main = "Figure 1.3: Correlations After Removal of Highly-Correlated Variables")
```
...
ets_rmse_tr <- sqrt(mean(SD_ets$residuals^2, na.rm = T))
...
autoplot(manual_forecast, series = "Forecast") +
  coord_cartesian(xlim = c(38000, 39488)) +
  ggtitle("Figure 2.11: Forecast from Regression with ARIMA(3,2,2) Erros")
```
