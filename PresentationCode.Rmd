...
Create Theme and USD Color Scheme
```{r}
Pres_theme <- function(base_size = 11, base_family = "") {
  ...
  theme_gray() %+replace%
    ...
    theme(
      ...
      panel.border = element_rect(color = "#303030", fill = NA),
...
missing.values %>%
  ggplot() + 
    geom_bar(aes(x=reorder(key, num.missing), y=num.missing), stat = 'identity', fill = USD_color) +
    labs(x='Variable', y="Number of missing values", title='Figure 1.1: Missing Values by Variable') +
    coord_flip() + Pres_theme()
```
...
ggplot(data = num2, aes(x= HourlyWetBulbTemperature, y=SDGE)) + 
  geom_point(color = USD_color) + 
  stat_poly_line(color = "white") + stat_poly_eq(color = "white") +
...
ScatPlotter.SD <- function(numvar){
  ggplot(num2, aes(x = num2[,numvar], y = SDGE)) +
    geom_point(shape = 1 , alpha = 0.2, color = USD_color) +
    stat_poly_line(color = "white") + stat_poly_eq(geom = "label", label.x = "middle", color = "white", fill = '#303030') +
...
p <- grid.arrange(grobs = Scatterplots)
ggdraw(p) + Pres_theme()
...
p <- grid.arrange(grobs = Scatter_paper)
ggdraw(p) + Pres_theme()
```
...
TS_plotter <- function(x){
  ...
  geom_line(aes(y = num_wdate[,x]), color = USD_color)+
...
p <- grid.arrange(grobs = TS_PHP) 
ggdraw(p) + Pres_theme()

p <- grid.arrange(grobs = TS_TWV)
ggdraw(p) + Pres_theme()

p <- grid.arrange(grobs = TS_paper)
ggdraw(p) + Pres_theme()
```
...
p <- grid.arrange(grobs = Hist_PHP)
ggdraw(p) + Pres_theme()

p <- grid.arrange(grobs = Hist_TWV)
ggdraw(p) + Pres_theme()

p <- grid.arrange(grobs = Hist_paper)
ggdraw(p) + Pres_theme()
```
...
```{r}
SD %>% filter(Date < '2019-01-03 18:00:00') %>% ggplot(aes(x = Date, y = SDGE)) + geom_line(color = USD_color) + ggtitle("Figure X.x: Seasonality in January") + Pres_theme()
SD %>% filter(Date < '2019-04-03 18:00:00') %>% filter(Date > '2019-03-31 18:00:00') %>% ggplot(aes(x = Date, y = SDGE)) + geom_line(color = USD_color) + ggtitle("Figure 1.14: Seasonality in April") + Pres_theme()
SD %>% filter(Date < '2019-07-03 18:00:00') %>% filter(Date > '2019-06-30 18:00:00') %>% ggplot(aes(x = Date, y = SDGE)) + geom_line(color = USD_color) + ggtitle("Figure 1.15: Seasonality in July") + Pres_theme()
SD %>% filter(Date < '2019-10-03 18:00:00') %>% filter(Date > '2019-09-30 18:00:00') %>% ggplot(aes(x = Date, y = SDGE)) + geom_line(color = USD_color) + ggtitle("Figure 1.16: Seasonality in October") + Pres_theme()
```
...
ggplot(SD_val, aes(x = Date)) +
  geom_line(aes(y = Naive), color = 'red', linewidth = 1, alpha = 0.6) +
  geom_line(aes(y = SDGE), color = USD_color, linewidth = 1) +
  Pres_theme() + ylab("SDGE Power Consumption (MWh)")

ggplot(SD_val, aes(x = Date, y = Naive_err)) +
  geom_line(color = USD_color) + 
  annotate("rect", xmin = min(SD_val$Date), xmax = max(SD_val$Date), ymin = -Naive_rmse_val, ymax = Naive_rmse_val, fill = "green", alpha = 0.2) +
  annotate("rect", xmin = min(SD_val$Date), xmax = max(SD_val$Date), ymin = Naive_rmse_val, ymax = Inf, fill = "red", alpha = 0.2) + 
  annotate("rect", xmin = min(SD_val$Date), xmax = max(SD_val$Date), ymin = -Naive_rmse_val, ymax = -Inf, fill = "red", alpha = 0.2) + 
  Pres_theme() +
  ylab("Naive Forecast Error (MWh)")
```
...
SD_madf %>% filter(Date > '2022-08-31 18:00:00') %>% 
  ggplot( aes(x = Date)) +
  geom_line(aes(y = SDGE), color = "red", linewidth = 1, alpha = 0.6) +
  geom_line(aes(y = DailyMA_2), color = USD_color, linewidth = 1) +
  ylab("SDGE Power Consumption (MWh)") + 
  Pres_theme()
```
Residual Plot
```{r}
SD_madf$Resid <- SD_madf$SDGE - SD_madf$DailyMA_2

SD_madf_val <- SD_madf %>% filter(Date > '2022-08-31 18:00:00') 


ggplot(SD_madf_val, aes(x = Date, y = Resid)) +
  geom_line(color = USD_color) +
  annotate("rect", xmin = min(SD_madf_val$Date), xmax = max(SD_madf_val$Date), ymin = -Naive_rmse_val, ymax = Naive_rmse_val, fill = "green", alpha = 0.2) +
  annotate("rect", xmin = min(SD_madf_val$Date), xmax = max(SD_madf_val$Date), ymin = Naive_rmse_val, ymax = Inf, fill = "red", alpha = 0.2) + 
  annotate("rect", xmin = min(SD_madf_val$Date), xmax = max(SD_madf_val$Date), ymin = -Naive_rmse_val, ymax = -Inf, fill = "red", alpha = 0.2) + 
  ylab("2 Hour MA Error (MWh)") +
  Pres_theme() 
```
...
Plot
```{r}
autoplot(val_forecast, ts.colour = USD_color, predict.colour = USD_color, conf.int.fill = 'red') +
...
SD_val$NNPred <- as.numeric(SD_NN.pred$mean)

ggplot(SD_val, aes(x = Date)) +
  geom_line(aes(y = SDGE), color = "red", linewidth = 1, alpha = 0.6) +
  geom_line(aes(y = NNPred), color = USD_color, linewidth = 1) +
  ylab("SDGE Power Consumption (MWh)") +
  Pres_theme()
```
...
Residual Plot
```{r}
SD_val$NNResid <- SD_val$SDGE - SD_val$NNPred

ggplot(SD_val, aes(x = Date, y = NNResid)) +
  geom_line(color = USD_color) +
  annotate("rect", xmin = min(SD_val$Date), xmax = max(SD_val$Date), ymin = -Naive_rmse_val, ymax = Naive_rmse_val, fill = "green", alpha = 0.2) +
  annotate("rect", xmin = min(SD_val$Date), xmax = max(SD_val$Date), ymin = Naive_rmse_val, ymax = Inf, fill = "red", alpha = 0.2) + 
  annotate("rect", xmin = min(SD_val$Date), xmax = max(SD_val$Date), ymin = -Naive_rmse_val, ymax = -Inf, fill = "red", alpha = 0.2) + 
  ylab("Neural Network Error (MWh)") +
  Pres_theme() 
```
...
Non-weather models
```{r}
Results_no <- Results_df %>% filter(Model %in% c("Naive", "2-Hour MA", "ETS"))

ggplot(Results_no, aes(x = reorder(Model, -Val_RMSE), y = Val_RMSE, color = BaselineComp_val)) + 
  geom_segment(aes(x=reorder(Model, -Val_RMSE), xend = reorder(Model, -Val_RMSE), y = 0, yend = Val_RMSE)) +
  scale_color_manual(values = c("gray77", USD_color, "firebrick4")) +
  geom_point(size = 9) + coord_flip() + labs(color = "Comparison to Baseline") +
  ylab("RMSE for Validation Data (MWh)") +  xlab("Model") + geom_text(aes(label = round(Val_RMSE)), color = "black", size = 2.5, fontface = "bold") +
  Pres_theme()
```

Weather Models
```{r}
Results_weather <- Results_df %>% filter(Model %in% c("Naive", "Auto ARIMA", "Manual ARIMA", "Seasonal ARIMA", "Neural Net."))

ggplot(Results_weather, aes(x = reorder(Model, -Val_RMSE), y = Val_RMSE, color = BaselineComp_val)) + 
  geom_segment(aes(x=reorder(Model, -Val_RMSE), xend = reorder(Model, -Val_RMSE), y = 0, yend = Val_RMSE)) +
  scale_color_manual(values = c("gray77", "firebrick4")) +
  geom_point(size = 9) + coord_flip() + labs(color = "Comparison to Baseline") +
  ylab("RMSE for Validation Data (MWh)") +  xlab("Model") + geom_text(aes(label = round(Val_RMSE)), color = "black", size = 2.5, fontface = "bold") +
  Pres_theme()
```

# Random Presentation Graphs

## SDGE Power Content

```{r}
Source <- c("Biomass", "Solar", "Wind", "Hydro", "Gas", "Nuclear", "Unspecified")
Percent <- c(0.9, 28.5, 15.2, 1.8, 29.6, 0.2, 23.9)
Content <- data.frame(Source, Percent)

ggplot(Content, aes(x = reorder(Source, Percent), y = Percent)) + geom_bar(stat = "identity", fill = USD_color) +
  xlab("Energy Source") + coord_flip() + Pres_theme()
```